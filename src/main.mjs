...
import {AstronomicalDataAPI} from 'grumptech-astrodata';
                        ...
                        case TRIGGER_TYPES.ScheduledTrigger: {
                            ...
                            if (_is.not.undefined(triggerConfig.astronomical_type)) {
                                ...
                                if (_is.not.undefined(triggerSettings.location)) {
                                    ...
                                    const validLoc = locValidator.IsLocationValid(triggerSettings.location);
        ...
        if (_is.existy(match) &&
            ...
            try {
                ...
                if (_is.existy(match.trigger)) {
                    ...
                    this._log(`TriggerStateChanged: Updating time remaining for trigger ${match.accessory.displayName}. active=${isActive} timeRemaining=${timeRemaining}`);
        ...
        if (_is.existy(match) &&
            ...
            try {
                ...
                if (_is.existy(match.trigger)) {
                    // Get the amount of time remaining
                    const timeRemaining = this._getTimeRemaining(match.trigger);
                    this._log(`TriggerStateNotify: Updating time remaining for trigger ${match.accessory.displayName}. active=${isActive} timeRemaining=${timeRemaining}`);
                    this._updateLightSensorService(match.accessory, SERVICE_INFO.TIME_REMAINING, {active: isActive, lightlevel: timeRemaining});
